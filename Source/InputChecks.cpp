...
#include "Headers/InputChecks.h"
	...
	double InputChecks::GenerateAccount() {
		...
		while (Sql.SQLQuery("SELECT Account FROM FinanceTrackerSheet;")) {	
			Loop++;
			Accounts.push_back(Sql.SQLQuery("SELECT Account FROM FinanceTrackerSheet;"));
		}
...
double WriteToFile::Add(double& In) {
		...
		SqlFunctions.SQLQuery(QueryChar);
...
void WriteToFile::Input() {
	...
	char SqlConnect::SQLQuery(const char* Query, std::vector<std::string> StringVec) {
